  ...
  <div class="modal-backdrop" @click.self="close">
    <div class="modal-content">
      <div class="modal-header">
        <h3>Edit Department: {{ department?.department?.name }}</h3>
        ...
        <form v-else @submit.prevent="saveDepartment">
          ...
          <div class="department-info">
            <div class="info-item">
              ...
              <span class="value">{{ department?.department?.name }}</span>
            </div>
            <div class="info-item">
              <span class="label">Building:</span>
              <span class="value">{{ department?.department?.building?.name || 'N/A' }}</span>
            </div>
          </div>
...
export default {
  name: 'DefaultEditDepartmentModal',
  components: {
    IconComponent
  },
  props: {
  ...
  emits: ['close', 'department-updated', 'department-removed'],
  setup(props, { emit }) {
    ...
    async function loadDepartment() {
      ...
      try {
        ...
        // Get the department
        const dept = defaultAreaCoverStore.getAssignmentById(props.departmentId);
        
        if (!dept) {
          formError.value = 'Department not found';
          return;
        }
        
        department.value = dept;
        startTime.value = dept.start_time;
        endTime.value = dept.end_time;
        color.value = dept.color;
        
        // Get porter assignments
        const assignments = defaultAreaCoverStore.getPorterAssignmentsByAreaId(props.departmentId);
        porterAssignments.value = assignments || [];
        
        // Initialize new porter assignment times
        newPorterAssignment.value.startTime = dept.start_time;
        newPorterAssignment.value.endTime = dept.end_time;
      } catch (error) {
        console.error('Error loading department:', error);
        formError.value = 'Failed to load department data';
      } finally {
        loading.value = false;
      }
    }
    ...
    async function saveDepartment() {
      ...
      try {
        ...
        const result = await defaultAreaCoverStore.updateDepartment(props.departmentId, updates);
        
        if (result) {
          emit('department-updated', result);
          close();
        } else {
          formError.value = 'Failed to update department';
        }
      } catch (error) {
    ...
    async function addPorter() {
      ...
      try {
        const result = await defaultAreaCoverStore.addPorterAssignment(
          props.departmentId,
          newPorterAssignment.value.porterId,
          newPorterAssignment.value.startTime,
          newPorterAssignment.value.endTime
        ...
        if (result) {
          ...
          newPorterAssignment.value = {
            porterId: '',
            startTime: department.value.start_time,
            endTime: department.value.end_time
          };
    ...
    async function updatePorter() {
      ...
      try {
        ...
        const result = await defaultAreaCoverStore.updatePorterAssignment(
    ...
    async function removePorter(assignment) {
      if (!confirm(`Are you sure you want to remove ${assignment.porter.first_name} ${assignment.porter.last_name} from this department?`)) {
        return;
      }
      ...
      try {
        const success = await defaultAreaCoverStore.removePorterAssignment(assignment.id);
    ...
    async function confirmDelete() {
      if (!confirm(`Are you sure you want to remove ${department.value?.department?.name} from the default assignments?`)) {
        return;
      }
      ...
      try {
        const success = await defaultAreaCoverStore.removeDepartment(props.departmentId);
        
        if (success) {
          emit('department-removed', props.departmentId);
          close();
        } else {
          formError.value = 'Failed to remove department';
        }
      } catch (error) {
