...
// Computed properties
const loading = computed(() => supportServicesStore.loadingAssignments);
const serviceAssignments = computed(() => supportServicesStore.serviceAssignments[props.shiftType] || []);
const availableServices = computed(() => {
  // Get all services
  const allServices = supportServicesStore.supportServices;
  
  // Get ids of services already assigned to this shift type
  const assignedServiceIds = serviceAssignments.value.map(a => a.service.id);
...
onMounted(async () => {
  ...
  if (!serviceAssignments.value.length) {
    await supportServicesStore.loadServiceAssignments(props.shiftType);
  }
...
// Watch for shifts in shift type
watch(() => props.shiftType, async () => {
  if (!serviceAssignments.value.length) {
    await supportServicesStore.loadServiceAssignments(props.shiftType);
  }
  
  // Update form defaults when shift type changes
  initializeFormDefaults();
});
...
async function confirmRemove(assignmentId) {
  if (confirm('Are you sure you want to remove this service assignment?')) {
    try {
      await supportServicesStore.removeServiceAssignment(assignmentId);
      showEditModal.value = false;
      selectedAssignment.value = null;
    } catch (error) {
